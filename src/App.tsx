...
const App: React.FC = () => {
  ...
  const handleConfirmDelete = async () => {
    if (deleteTodoId !== null) {
      try {
        const response = await fetch(`https://deploying-backend-1-m5e0.onrender.com/api/todos/${deleteTodoId}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://deploying-backend-1-m5e0.onrender.com/api/todos');
        if (!response.ok) {
